...
class MyDomain():
    def __init__(self, mymesh, mri_para):
        ...
        self.D0 = 3e-3
        self.kappa_e_scalar = 3e-3

    def WeakPseudoPeridicMarker(self):        
        if self.gdim==2:
            pmk = self.kappa_e_scalar/self.hmin*Expression("(x[0]<xmin+eps || x[0]>xmax-eps)*p0 || (x[1]<ymin+eps || x[1]>ymax-eps)*p1", 
        ...
        if self.gdim==3:
            pmk = self.kappa_e_scalar/self.hmin*Expression("(x[0]<xmin+eps || x[0]>xmax-eps)*p0 || (x[1]<ymin+eps || x[1]>ymax-eps)*p1 || (x[2]<zmin+eps || x[2]>zmax-eps)*p2", 
